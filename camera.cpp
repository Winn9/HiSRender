...
void Camera::move_position(Camera* camera, float x, float y, float z)
{
	this->position.x += x;
	this->position.y += y;
	this->position.z += z;
...
// property retrieving
Vector3f Camera::update_set_position(float x, float y, float z)
{
	Vector3f p = get_position();
	p.x += x;
	p.y += y;
	p.z += z;
	return p;
...
Matrix4f Camera::get_view_matrix()
{
	...
	Matrix4f m = lookat(position, target, up);
	Matrix4f ret = Matrix4f::identity();
	return m;
}

Matrix4f Camera::get_proj_matrix()
{
	Matrix4f m = perspective(FOVY, aspect, NEAR, FAR);
	return m;
}
