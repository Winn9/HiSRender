...
Model::Model(const char* filename) : verts_(), faces_(), norms_(), uv_(), diffusemap_(), normalmap_(), specularmap_()
{
	...
	while (!in.eof())
	{
		...
		char trash;
		if (!line.compare(0, 2, "v "))	// v: Geometric vertices
		{
			iss >> trash;
			Vector3f v;
		...
		else if (!line.compare(0, 3, "vn "))	// vn: Vertex normals
		{
			iss >> trash >> trash;
			Vector3f n;
		...
		else if (!line.compare(0, 3, "vt "))	// vt: Texture vertices
		{
			iss >> trash >> trash;
			Vector2f uv;
		...
		else if (!line.compare(0, 2, "f "))	// f: face
		{
