...
Matrix4f GameObject::GetModelMatrix()
{
	Matrix4f m_translation = translate(transform.position.x, transform.position.y, transform.position.z);
	Matrix4f m_rotate = rotate_z(TO_RADIANS(transform.rotation.z)) * rotate_x(TO_RADIANS(transform.rotation.x)) * rotate_y(TO_RADIANS(transform.rotation.y));
	Matrix4f m_scale = scale(transform.scale.x, transform.scale.y, transform.scale.z);
